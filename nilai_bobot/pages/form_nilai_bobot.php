...
if (Route::is_ajax()) {
    ...
    if ($p_act == "edit" && !empty($id)) {
        ...
        $sqlKriteria = "select id, nama_kriteria from kriteria where id in (select id_kriteria from nilai_bobot where id_kriteria = $id)";
        $queryKriteria = mysqli_query($conn->connect(), $sqlKriteria);

        // Ambil Data nilai_bobot_kriteria sesuai data yang mau diedit
        $sql = "select*from nilai_bobot where id_kriteria = $id and 1=1";
        ...
        $sqlKriteria = "select id, nama_kriteria from kriteria where id not in (select id_kriteria from nilai_bobot)";
        $queryKriteria = mysqli_query($conn->connect(), $sqlKriteria);
